...
import { SearchBar, Text, Tile, Avatar, Card, CheckBox, Input } from 'react-native-elements';
...
export default Personas = ({ route, navigation }) => {
	...
	useEffect(() => {
		...
		console.log('=> Parametros: ', route.params.item);
		setMunicipioObligado(route.params.item.idMunicipio);

		if(route.params.item.NombreGrupo.startsWith('R')) setViewAll(true);
		...
		if(!viewAll)
		{
			if (search) {
				data = {
					...
					sorted: [],
					filtered: [
					{ id: "et_aprobadoscomite.idMunicipioC", value: route.params.item.idMunicipio}],
				};
			} else {
				data = {
					page: dataPage.page,
		...
		fetch('https://apicovid.chiapasavanza.com/api/getAprobados', requestInfo)
			.then((response) => {
				if (response.ok) {
	...
	btnSeleccionar = (item) => {
		...
		if(item.EstatusExpediente=== "COMPLETO")
		{
			if(route.params.item.NombreGrupo.startsWith('R'))
			{
				//console.log('=> Grupo empieza con R.', route.params.item.NombreGrupo);
				//return true;
				...
				<Card key={`${item.id}`} title={`${item.NombreC} ${item.PaternoC} ${item.MaternoC}`}>
					<View style={styles.rowDeep}>
						...
						>
							...
							<View style={{ flex: 1, flexDirection: 'row', marginLeft: 5 }}>
								<Text style={{ flexWrap: 'wrap', alignItems: 'flex-start' }}>EstatusExpediente</Text>
								<Text style={{ fontWeight: 'bold' }}> {`${item.EstatusExpediente}`}</Text>
							</View>
						</View>
					</View>
				</Card>
    ...
    */
	...
	return (
		<Center>
			...
			<CheckBox
				center
				title='Ver de todos los municipios'
				checkedIcon='dot-circle-o'
				uncheckedIcon='circle-o'
				checked={viewAll}
				onPress={() => {setViewAll(!viewAll); updateSearch()}}
			/>
				{!buscando && (
					<Input
						placeholder="Buscar..."
